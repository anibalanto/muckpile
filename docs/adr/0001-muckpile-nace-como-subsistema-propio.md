...
multitask/
  ...
  acc/                              ← otro proyecto, sin relación con el primero — mismos cuatro nombres reservados
    backlog/
      sprint/
...
**Y la raíz de un proyecto tiene exactamente cuatro nombres reservados: `base/`, `backlog/`, `to-work/` y `query/`.** Nada más vive ahí suelto. Antes una vista de trabajo (`SGE-9876/`) quedaba al mismo nivel que `backlog/` — funcionaba, pero dejaba abierta la posibilidad de que la clave de un ítem coincidiera algún día con un nombre reservado. Con `to-work/` como tercer contenedor —y `query/` como cuarto, para las consultas—, la raíz es siempre esas carpetas fijas y nada dinámico vive ahí: cualquier nombre que varíe —clave de ítem, nombre de sprint, nombre de consulta— está siempre un nivel adentro de una de ellas.
...
|---|---|---|
| `init` | Crea un proyecto: `<proyecto>/.muckpile/`, un `muckpile.toml` para completar, y `base/`, `backlog/`, `to-work/`, `query/`. Es el único comando que crea `.muckpile/` (decisión 6). | `$ cd multitask && muckpile init sge` |

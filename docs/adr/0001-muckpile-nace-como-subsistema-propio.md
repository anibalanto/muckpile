...
| `unlink` de un tipo cuyas dos frases son la misma —`Relates`: `relates to` de ida y de vuelta— | `falta spec` | Lo decide el código: `unlink a relates_to b` busca el link con `a` de ida; si se creó al revés —desde `b`, o en Jira—, dice que no existe, aunque el header de `a` muestre `relation.relates_to: [b]`. Probar las dos direcciones cuando la frase es la misma de los dos lados sería decidir, y no está dicho |

...
fn applies_the_transition_leading_to_the_requested_status() {
    ...
    let outcome = transition("ACC-355", "Finalizada", &provider, approved).unwrap();
...
fn reports_what_the_workflow_does_reach_when_the_target_is_not_one_of_them() {
    ...
    let outcome = transition("ACC-360", "Finalizada", &provider, approved).unwrap();
...
fn refuses_an_id_with_characters_a_path_cannot_carry() {
    let provider = FakeProvider::new();
    assert!(transition("../escape", "Finalizada", &provider, approved).is_err());
}
